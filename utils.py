...
def get_volunteer_numbers(message):
    ...
    return [phone_numbers_by_language(language), language]

def format_acceptance_message(event_id, language):
    return """Hi Polyglot! Someone needs your translation assistance in {}. \nRespond back with the number {} to accept.""".format(language, event_id)
...
def format_send_user_message(message):
    return "Thanks for accepting!  Here's the message and phone number at which to followup: {}".format(message)
